...
const HomePage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    initializeDatabase()
      ...
      .catch((error) => {
        ...
        setError('ブラウザストレージの初期化に失敗しました。ブラウザの設定を確認してください。');
      });
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isDbInitialized) {
      setError('データベースが初期化されていません。ページを再読み込みしてください。');
      setIsLoading(false);
      return;
    }
    ...
    try {
      ...
      // AnalysisService を使用して分析を開始
      const analysisId = await AnalysisService.startAnalysis(validatedUrl);
...
const ConnectionStatus: React.FC = () => {
  ...
  const [browserStorageStatus, setBrowserStorageStatus] = React.useState<'checking' | 'supported' | 'unsupported'>('checking');

  React.useEffect(() => {
    // ブラウザストレージサポート確認
    const checkBrowserStorage = () => {
      try {
        const isSupported = !!(window.indexedDB && window.localStorage);
        setBrowserStorageStatus(isSupported ? 'supported' : 'unsupported');
      } catch (error) {
        setBrowserStorageStatus('unsupported');
      }
    };
    ...
    const interval = setInterval(checkServerConnection, 30000); // 30秒ごとにサーバーチェック
  ...
  const server = serverConfig[serverStatus];
  const storage = storageConfig[browserStorageStatus];

  return (
    <div className="flex flex-col gap-2">
      <div className="inline-flex items-center px-4 py-2 rounded-xl text-sm font-medium border border-slate-600/50 bg-slate-800/40 backdrop-blur-sm shadow-sm">
        <div className={cn("w-2 h-2 rounded-full mr-2", server.dot)}></div>
        <span className={server.color}>{server.text}</span>
      </div>
      <div className="inline-flex items-center px-4 py-2 rounded-xl text-sm font-medium border border-slate-600/50 bg-slate-800/40 backdrop-blur-sm shadow-sm">
        <div className={cn("w-2 h-2 rounded-full mr-2", storage.dot)}></div>
        <span className={storage.color}>{storage.text}</span>
      </div>
    </div>
  );
};
